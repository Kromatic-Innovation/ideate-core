...
//
// The evaluator is INJECTABLE and evaluator-agnostic. Kromatic's `plenum` is the
// intended first evaluator; `plenumToFeedback` below maps plenum's shipped
// spawn/score output ({verdict, message, dealKillers[]}, plenum#7) onto the
// feedback-in contract. The live plenum↔ideate wiring is the consumer's job
// (social-loop) — this module only defines the contract + the loop.
//
...
/**
 * Adapter: map plenum's shipped per-idea output ({verdict, message,
 * dealKillers[]}, plenum#7) onto the feedback-in contract. plenum verdicts map
 * pass→keep, revise→revise, kill→kill; `message` becomes a keepReason; deal
 * killers pass straight through. `ideaId` must be supplied alongside (plenum
 * scores an idea the caller identifies).
 *
 * @param {Array<{ideaId:string, verdict:string, message?:string, dealKillers?:string[],
 *          scores?:object}>} plenumVerdicts
 * @returns {object} feedback-in contract
 */
export function plenumToFeedback(plenumVerdicts, poolDirectives = []) {
  const verdicts = (Array.isArray(plenumVerdicts) ? plenumVerdicts : []).map((p) => ({
