// ideate-core.mjs — domain-agnostic ideation engine.
//
// A reusable multi-agent, multi-round idea generator developed by Kromatic for
// internal use, then open sourced. It owns ONLY the generic machinery:
...
//     persona: string,               // the agent persona label that produced it
 ...
 *                                        { id, persona, stance, temperature, strategy, ideasPerAgent, model, effort }.
 *                                        Missing fields fall back to DEFAULT_PERSONAS by index. `effort` is a
 *                                        pass-through with no default — absent stays distinguishable from an
 *                                        explicit value; the library does not interpret or validate it.
 ...
 *   @param {function}  [deps.normalizeExtra] (raw, ctx)=>object|null. ctx = {persona,
 *                                        agentId, model, round, n, context, origin}.
 ...
 *   @param {function}  [deps.onAgentError] (err, {agentId, round})=>void. Called for
 *                                        each agent whose call throws, returns a bad
 *                                        reply, or has no client, ON EVERY ROUND (round 1
 *                                        and any build-on round). Makes the swallowed
 *                                        per-agent failure observable; never changes the
 *                                        no-throw robustness contract. Failures are ALSO
 *                                        recorded by default (whether or not this is
 *                                        supplied) in meta.agentErrors (all rounds) --
 *                                        so total round-1 failure is distinguishable from
 *                                        a legitimately empty pool without opting in to
 *                                        this callback. meta.agentsAttempted / agentsFailed
 *                                        cover round 1 ONLY; see meta.agentErrors below for
 *                                        the exact relationship between the two.
 *                                        CONTRACT (ideate-core#157): this callback MAY throw
 *                                        SYNCHRONOUSLY. Such a throw is CONTAINED -- it can
 *                                        never reject the ideateCore() call -- and is
 *                                        recorded as its own
 *                                        `kind: "callback"` entry in meta.agentErrors,
 *                                        distinct from the `kind: "agent"` entry for the
 *                                        underlying failure that triggered the call (both
 *                                        can exist for one agent: it failed for its own
 *                                        reason AND the caller's handler then threw). The
 *                                        agent's own entry is always recorded before this
 *                                        callback runs, so a throwing callback can never
 *                                        suppress the record it was reacting to. An ASYNC
 *                                        callback that REJECTS is also contained -- it can
 *                                        never crash the process via an unhandled rejection
 *                                        -- but a rejection is NOT recorded in
 *                                        meta.agentErrors: it settles after this call
 *                                        returns, possibly after ideateCore has already
 *                                        returned meta to the caller, so there is no point
 *                                        to record it into without either blocking the run
 *                                        on the callback or mutating an object the caller
 *                                        already holds. Only a synchronous throw is
 *                                        recorded.
...
export async function ideateCore(input = {}, deps = {}) {
  ...
  const callerOnAgentError = typeof deps.onAgentError === "function" ? deps.onAgentError : null;
  ...
  // ── Default failure recording (ideate-core#154) ─────────────────────────────
  // agentsFailed (below) makes total failure COUNTABLE, but a count alone still
  // forces a caller to opt in to `deps.onAgentError` to learn WHY. Record every
  // failure's reason by default -- unconditionally, whether or not the caller
  // supplied their own onAgentError -- so `meta.agentErrors` always carries the
  // reason. The caller's own onAgentError (if any) still fires exactly once per
  // failure, unshadowed and undoubled: this wraps it rather than replacing it.
  const agentErrors = [];
  const onAgentError = (err, ctx) => {
    // Record the agent's own failure FIRST (ideate-core#155's ordering pin,
    // still true here): a throwing caller callback below must never be able
    // to suppress the record it was reacting to.
    agentErrors.push({
      kind: "agent",
    ...
    if (callerOnAgentError) {
      // ideate-core#157: onAgentError is a diagnostic hook -- it exists so a
      // caller can OBSERVE a failure, and must never be able to CAUSE one. A
      // throw here is CONTAINED (never escapes to reject the Promise.all this
      // runs inside) and recorded as a DISTINCT entry, `kind: "callback"`, so
      // a consumer counting agent failures (kind: "agent") never mistakes a
      // broken diagnostic hook for one. Deliberately keyed `agentRound`, not
      // `round` -- the ideate-core#155 invariant
      //   agentErrors.filter(e => e.round === 1).length === agentsFailed
      // filters on `.round` with no kind check, so a callback entry sharing
      // that key would over-count and break it the moment a callback throws
      // in round 1.
      try {
        const result = callerOnAgentError(err, ctx);
        // An async onAgentError (the single most natural way to write this
        // hook -- `async (e) => await logToService(e)`) can REJECT instead
        // of throwing synchronously. try/catch does not catch a rejection,
        // and an unhandled rejection is process-fatal under Node's default
        // `--unhandled-rejections=throw`. Attach a handler so it can never
        // crash the process.
        //
        // Deliberately NOT recorded in meta.agentErrors, and deliberately
        // NOT awaited:
        //   - awaiting it here would block ideateCore's return on a
        //     caller's logger -- exactly the failure mode this containment
        //     exists to prevent, just moved from "throws" to "hangs".
        //   - a rejection settles AFTER this synchronous call returns, which
        //     may be after ideateCore has already returned `meta` to the
        //     caller. `agentErrors` is the same array object referenced by
        //     the already-returned `meta.agentErrors` -- pushing into it
        //     later would silently mutate an object the caller already
        //     holds, which is corruption, not observability.
        // So: contained (no crash), but unrecorded. Only a SYNCHRONOUS throw
        // (below) is recorded as `kind: "callback"`.
        ...
        agentErrors.push({
          kind: "callback",
          agentId: ctx && ctx.agentId,
          agentRound: ctx && ctx.round,
          message: errorMessage(callbackErr),
        });
      }
    }
  };
  ...
  const r1Results = await Promise.all(
    agents.map((agent) => {
      ...
      if (!complete) {
        // No client resolvable for this agent — still a failed agent (it will
        // produce zero candidates), so surface it exactly like a thrown call.
        // (onAgentError is always a function here — see its definition above
        // — so no `if` guard is needed.)
        onAgentError(new Error(`no client resolvable for model "${agent.model || "(default)"}"`), {
          agentId: agent.id,
          round: 1,
        });
      ...
      return safeComplete(
        ...
        onAgentError,
        { agentId: agent.id, round: 1 },
      );
    }),
  );
  // A null round-1 result means that agent produced nothing (threw, returned a
  // bad reply, or had no client). Count it so an all-failed run is OBSERVABLE in
  // meta instead of a silent empty pool.
  const agentsFailed = r1Results.filter((r) => !r).length;
  ...
  // Round 1 always executes; build-on rounds may exit early (no round-2 builder,
  // or an empty pool). Track how many ACTUALLY ran so meta reports executed
  // rounds, not configured ones — meta.sharing is the field asserting real
  // brainwriting occurred, so it must not claim a round that never ran.
  let roundsRun = 1;
  for (let round = 2; round <= maxRounds; round++) {
    ...
    const sharedPool = rc.sharing === "pool" ? dedupe(candidates.slice(), dedupeKey) : null;

    byAgentRound[round] = {};
    const inputs = agents.map((agent) => {
      const seeds = rc.sharing === "pool" ? sharedPool : byAgentRound[round - 1][agent.id] || [];
    ...
    const results = await Promise.all(
      inputs.map((inp) => {
        ...
        if (!complete) {
          // No client resolvable for this round's agent — parity with the
          // round-1 no-client path above. Without this, a caller-supplied
          // deps.resolveClient that goes stateful between rounds (e.g. a
          // circuit breaker that opens after round 1) drops round-2 agents
          // with NO agentErrors entry and no agentsFailed increment: a
          // silent failure, which is exactly the class this issue exists to
          // close.
          onAgentError(
            new Error(`no client resolvable for model "${inp.agent.model || "(default)"}"`),
            { agentId: inp.agent.id, round },
          );
          return Promise.resolve(null);
        }
        return safeComplete(
  ...
  const ideasPerAgentValues = agents.map((a) => a.ideasPerAgent);
  const ideasPerAgentUniform =
    ideasPerAgentValues.length > 0 &&
    ideasPerAgentValues.every((v) => v === ideasPerAgentValues[0]);
  const meta = {
    independence: "blind",
    agentCount: agents.length,
    // Observability for the silent-empty-pool hazard: how many round-1 agents
    // were attempted and how many produced nothing (threw / bad reply / no
    // client). agentsFailed === agentsAttempted means every agent failed and the
    // pool is empty for that reason, NOT because the models had no ideas.
    agentsAttempted: agents.length,
    agentsFailed,
    // Failure reasons, recorded by default (ideate-core#154) -- always
    // populated regardless of whether the caller supplied deps.onAgentError.
    // NOT scoped the same as agentsFailed: agentsFailed counts ONLY round-1
    // nulls (see r1Results above), while agentErrors accumulates failures
    // from EVERY round (the same onAgentError wrapper is passed into every
    // round's safeComplete call). The invariant that relates them:
    //   agentErrors.filter(e => e.round === 1).length === agentsFailed
    // agentsFailed === agentsAttempted with candidates.length === 0 is total
    // round-1 failure; agentErrors (all rounds) says why.
    // ideate-core#157: entries carry `kind: "agent"` (the failure this array
    // was created for) or `kind: "callback"` (the caller's own onAgentError
    // threw while reacting to one). Callback entries key their round as
    // `agentRound`, NOT `round`, precisely so the invariant above -- which
    // filters on `.round` with no kind check -- keeps holding even when a
    // round-1 callback throws.
    agentErrors,
    // True per-agent ideasPerAgent distribution. A single scalar is misleading
    // when a panel mixes values (resolveAgents supports per-agent ideasPerAgent),
    // so report the scalar ONLY when uniform, else null, and always ship the
    // per-agent array.
    ideasPerAgent: ideasPerAgentUniform ? ideasPerAgentValues[0] : null,
    ideasPerAgentByAgent: ideasPerAgentValues,
    crossProvider: new Set(agents.map((a) => a.model || "").filter(Boolean)).size > 1,
    // maxRounds = configured; roundsRun = executed. sharing reflects the rounds
    // that ACTUALLY ran (a no-round-2 run reports ["blind"], not ["blind","pool"]).
    maxRounds: resolveMaxRounds(deps),
    roundsRun,
    sharing: Array.from({ length: roundsRun }, (_, i) => roundConfig(i + 1, deps).sharing),
 ...
 *   2. the default persona panel of deps.agentCount agents (default 5).
 * Every returned agent has: { id, persona, stance, temperature (numeric),
 * strategy, ideasPerAgent, model }. `effort` is optional and only present, as
 * `undefined` or an explicit value, on an agent resolved from an explicit
 * deps.agents entry (precedence 1) — the default persona panel (precedence 2)
 * never sets it. Either way it reads as `undefined` unless the caller set it —
...
export function resolveAgents(deps = {}) {
  ...
  if (Array.isArray(deps.agents) && deps.agents.length) {
    raw = deps.agents.map((a, i) => {
      ...
      return {
        ...
        model: spec.model,
        id: spec.id,
        // Optional reasoning-effort override, no default — absent must stay
        // distinguishable from an explicit "low" (ideate-core#146). DEFAULT_PERSONAS
        // never carries an `effort` key, so an unset spec.effort passes through as
        // undefined rather than falling back to any base value.
        effort: spec.effort,
      };
    });
  } else {
  ...
  // `used` tracks EVERY id actually assigned — including generated suffixes — so a
  // later agent explicitly named in the collision-suffix format (e.g. `a-2`)
  // cannot silently reuse an id the de-collision loop already produced. The
  // suffix loop increments until it lands on a genuinely free id, rather than
  // incrementing once and trusting it (ideate-core#95).
  const used = new Set();
  const counter = new Map(); // base id -> highest suffix already tried
  return raw.map((a) => {
    const base = a.id && typeof a.id === "string" && a.id.trim() ? a.id.trim() : a.persona;
    let id = base;
    if (used.has(id)) {
      let n = counter.get(base) || 1;
      do {
        n += 1;
        id = `${base}-${n}`;
      } while (used.has(id)); // keep going until the suffixed id is actually free
      counter.set(base, n);
    } else if (!counter.has(base)) {
      counter.set(base, 1);
    }
    used.add(id); // register the FINAL id, not just the base
    return { ...a, id };
  });
}
...
export function roundConfig(round, deps = {}) {
  ...
  const sharing =
    spec.sharing === "blind" || spec.sharing === "pool" ? spec.sharing : isFirst ? "blind" : "pool";
  const incubation = typeof spec.incubation === "boolean" ? spec.incubation : !isFirst; // fresh context on build-on rounds
  const buildOnDirective = isFirst
    ? null
    : spec.buildOnDirective ||
      (typeof deps.buildOnDirective === "string"
        ? deps.buildOnDirective
        : DEFAULT_BUILD_ON_DIRECTIVE);
...
export function makeResolveComplete(deps = {}) {
  ...
  const resolveClient = typeof deps.resolveClient === "function" ? deps.resolveClient : null;
 ...
 *   @param {string}   [deps.persona]      bucket label for human ideas (default "human")
 *   @param {string}   [deps.temperature]  DEPRECATED alias for deps.persona, kept for
 *                                        back-compat with existing callers; deps.persona
...
export function foldHumanIdeas(humanIdeas, deps = {}) {
  ...
  const normalizeExtra =
    typeof deps.normalizeExtra === "function" ? deps.normalizeExtra : passthroughExtra;
  const makeId = typeof deps.makeId === "function" ? deps.makeId : defaultMakeId;
  const persona = deps.persona || deps.temperature || "human";
  ...
  for (const entry of humanIdeas) {
    ...
    const explicitId = raw.id && typeof raw.id === "string" && raw.id.trim() ? raw.id.trim() : null;
...
function defaultMakeId(ctx) {
  const slug = ctx.context && typeof ctx.context.slug === "string" ? ctx.context.slug : "idea";
  // Key the id on the AGENT id, not the persona label. `resolveAgents` already
  // de-collides two agents that share a persona into distinct ids
  // (`pragmatist` / `pragmatist-2`); keying on `ctx.persona` (the persona
  // label) here would have thrown that away and produced colliding ids, which
  // downstream byId maps (feedback/converge) silently drop. The human path is
  // unaffected: it passes `agentId: "human"` and `persona: "human"`, so
  // `${slug}-human-r0-N` is unchanged.
  return `${slug}-${ctx.agentId}-r${ctx.round}-${ctx.n}`;
}

// ── Error message extraction ─────────────────────────────────────────────────

// A caller's injected `complete` (or a third-party SDK underneath it) may
// throw a non-Error value (a plain string, a plain object, null, ...) — we
// don't control that. `err.message` alone loses the reason entirely for
// those (the key is simply absent, or is `null`), which would defeat the
// whole point of recording a reason. Prefer a real .message; otherwise try
// to render the thrown value informatively (JSON for a plain object) before
// falling back to String() for anything JSON can't handle (a symbol, a
// circular structure, etc.).
function errorMessage(err) {
  ...
  if (err && typeof err === "object") {
    try {
      ...
      // fall through to String() below (e.g. a circular structure)
    }
  }
  return String(err);
}
...
async function safeComplete(complete, req, onError, errCtx) {
  // Robustness contract (CONTRIBUTING.md "Robustness over strictness"): a bad
  // per-agent reply must NEVER sink the run — always resolve to null, never
  // throw. The optional onError callback makes the swallowed failure OBSERVABLE
  // (the ideate-core#90 gap) without changing that contract.
  let res;
  try {
    res = await complete(req);
  } catch (err) {
    if (typeof onError === "function") onError(err, errCtx || {});
    return null;
  }
  if (!res || res.ok !== true || typeof res.text !== "string") {
    if (typeof onError === "function") {
      onError(new Error("model reply not ok / missing text"), errCtx || {});
    }
    return null;
  }
  return res;
