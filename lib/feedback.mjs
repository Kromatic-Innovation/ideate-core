...
export async function runFeedbackLoop(candidates, deps = {}, opts = {}) {
  ...
  // Resolve the panel once so regeneration can recover the ORIGINATING agent's
  // numeric sampling temperature by original.agentId — candidates carry no
  // temperature field of their own (ideate-core#166). Keyed on agentId — sound
  // now that ids are unambiguous (ideate-core#87).
...
async function regenerateOne(original, verdict, poolDirectives, ctx) {
  ...
  // Honor deps.maxTokens and the originating agent's tuned temperature,
  // keeping the previous hardcoded values ONLY as fallbacks. An agent that
  // does not resolve (e.g. a human-origin candidate, or a pool from another
  // panel) falls back to 0.7.
  const agent = ctx.agentsById ? ctx.agentsById.get(original.agentId) : null;
  const temperature = agent ? agent.temperature : 0.7;
