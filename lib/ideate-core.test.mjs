...
test("all agents failing is observable in meta (agentsFailed) — no throw (#90)", async () => {
...
test("partial agent failure is counted, not hidden (#90)", async () => {
...
test("de-collision handles an explicit name colliding with a generated suffix (#95)", () => {
...
test("de-collision still handles the plain wrapped/repeated case (#95 regression)", () => {
  assert.deepEqual(
...
test("meta reports EXECUTED rounds, not configured ones (#91)", async () => {
...
test("meta does not summarize a mixed ideasPerAgent panel as one scalar (#91)", async () => {
  const complete = personaClient();
  const { meta } = await ideateCore(
    { context: { slug: "demo" } },
    {
...
test("meta reports a uniform ideasPerAgent as the scalar plus the array (#91)", async () => {
  const complete = personaClient();
  const { meta } = await ideateCore(
    { context: { slug: "demo" } },
    { buildRound1Prompt: personaPrompt, complete, agentCount: 3 },
