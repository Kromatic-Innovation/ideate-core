...
//
// The evaluator is INJECTABLE and evaluator-agnostic. Kromatic's `panelist` is the
// intended first evaluator; `panelistToFeedback` below maps panelist's shipped
// spawn/score output ({verdict, message, dealKillers[]}, panelist#7) onto the
// feedback-in contract. The live panelist↔ideate wiring is the consumer's job
...
/**
 * Adapter: map panelist's shipped per-idea output ({verdict, message,
 * dealKillers[]}, panelist#7) onto the feedback-in contract. panelist verdicts map
 * pass→keep, revise→revise, kill→kill; `message` becomes a keepReason; deal
 * killers pass straight through. `ideaId` must be supplied alongside (panelist
 ...
 */
export function panelistToFeedback(panelistVerdicts, poolDirectives = []) {
