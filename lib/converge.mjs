 ...
 *
 * COMPLEXITY (n = pool size). Every stage past embedding is at least quadratic, so
 * cost grows fast if a caller raises `agentCount` / `ideasPerAgent` for a large n:
 *   - `semanticDedupe`  — O(n²): each item is compared against the kept set.
 *   - `clusterByEmbedding` — depends on the `cluster.k` mode:
 *       • `k: "auto"` (default) → `leaderCluster`, O(n·k) where k = #clusters found.
 *         Prefer this for large pools — it avoids the cubic path below.
 *       • `k: <number>` → `agglomerative`, O(n³) (an O(n²) pairwise scan inside an
 *         O(n) merge loop). Fine for small n; expensive for large n with a fixed k.
 *   - `scoreAxes`     — O(n²): the quadratic half is NOVELTY, which compares each
 *                       item against every OTHER item and keeps the MAX cosine
 *                       similarity — no ranking, no normalization, nothing scaled
 *                       against the pool's spread. FEASIBILITY never sees the pool
 *                       at all: it is one injected `scoreFeasibility(item)` call
 *                       per item, and null when no scorer is injected. The two
 *                       axes are computed by different means and never combined.
 *   - `poolDiversity` — O(n²): mean pairwise cosine distance.
 * The default pool (5 × 6 × 2 = 60) is ~microseconds; the exposure is a caller who
 * raises the panel size AND passes a fixed numeric `k`. No cap is imposed here —
 * choose `k: "auto"` for large pools rather than bounding n by a guess.
 *
...
export async function convergePool(candidates, deps = {}) {
  const cfg =
    (deps.convergence && typeof deps.convergence === "object" ? deps.convergence : {}) || {};
