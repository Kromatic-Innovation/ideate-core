...
- **Independent multi-agent round 1** — N agents (default 5), each a separate blind model call. Diversity is _engineered_ via per-agent levers — **persona** (default: pragmatist / contrarian / domain-expert / outsider-analogy / visionary), **prompt strategy** (chain-of-thought), and, where the model supports it, **temperature**. "Be diverse" alone fails; persona beats temperature as a lever (Wang et al. 2023; Meincke et al. 2024). Note: current Anthropic frontier models (Opus 5, Sonnet 5, Opus 4.8/4.7, Fable 5) reject `temperature`/`top_p`/`top_k`; Haiku 4.5 still accepts them. The example adapters strip a rejected sampling param and warn ([docs/ideation-method.md §2](docs/ideation-method.md)), so **persona is the primary structural lever on frontier models**.
...
- **Per-agent `effort`** — an optional `effort` field on an agent spec (`deps.agents[i].effort`), passed straight through to that agent's `complete(req)` call as `req.effort`. It is assigned, not merged with a fallback the way `persona`/`model` are (`spec.X || base.X`) — so `effort: null`, `0`, or `""` reach `req.effort` unchanged instead of falling back to a default, and an agent that never set it stays distinguishable (`undefined`) from one that explicitly set a falsy value. The engine does not validate or interpret `effort` — it is opaque cargo; the meaningful ladder (e.g. `low`/`medium`/`high`/`xhigh`/`max`) is defined by whatever adapter or provider reads it. Both bundled adapters forward it: `integrations/subagent-dispatch` includes it in the default `mapRequest` task shape, `integrations/headless-cli` forwards it as `--effort <value>`.
