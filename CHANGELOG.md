...
max`) is identical to the API's `output_config.effort`, so it passes through
  ...
  own `options.args` already contains `--model`/`--effort`, the per-agent
  request field wins: it replaces the caller's flag+value pair rather than
  being appended alongside it, so the outcome never depends on argument-array
  order. Documented in `skills/adapter-authoring/SKILL.md`'s request-field
  table alongside the `subagent-dispatch` adapter's fuller forwarding.
...
- **Test coverage for the shipped example prompt builders (ideate-core#99).** The two
...
- **Regeneration runs concurrently; convergence complexity documented (ideate-core#98).**
...
- **Agent failure counters — the silent-empty-pool is now observable (ideate-core#90).**
...
round})` callback. `runFeedbackLoop` gains `feedback.evaluatorFailures` and a
  ...
  swallowed.
- **Adapter-side strip-and-warn for rejected sampling params (ideate-core#89).** Current
...
- **Agent id de-collision could still collide with an explicit name (ideate-core#95).**
  ...
  ID bug (ideate-core#87). Every assigned id is now registered and the suffix loop increments
  until it finds a genuinely free id, yielding `["a","a-2","a-2-2"]`.
- **Regeneration bypassed `deps.maxTokens` and the agent's temperature (ideate-core#92).**
  ...
  `original.agentId` (sound now that ids are unambiguous, ideate-core#87), keeping `0.7` /
...
- **CLI crashed at module load on any path containing a space (ideate-core#88).**
...
- **Candidate ID collision when two agents share a persona (ideate-core#87).** `defaultMakeId`
...
cross-repo reusable workflow was **withdrawn** (ideate-core#66): a public repo cannot call
...
- Gitignored the generated `.agents/`, `.codex/`, and `.zenodotus/` directories
  (cwc#369, ideate-core#65).
- Pinned the release workflow to the `npm@11` line for OIDC trusted publishing,
  mirroring tickle-stick (ideate-core#64). Supersedes two earlier incorrect attempts: an
  `NPM_TOKEN` fallback (ideate-core#63) and a Node 24 bump (ideate-core#62), both aimed at the same
...
> did: ideate-core#46 removed the `deps.temperatures` back-compat path and its exported
...
- **Packaging** (ideate-core#47) — exclude `docs/assets/hero.png` (~1.5 MB) from the
...
- **Legacy temperature-stance panel** (ideate-core#46) — dropped the pre-S1
...
- **`./feedback` export renamed** (ideate-core#48) — the worked-example adapter exported as
...
- **Security docs** (ideate-core#45, docs only) — `SECURITY.md` now calls out the CLI
  ...
  engine's.
- **Docs hygiene** (ideate-core#44, docs only) — split the `[0.1.0]` changelog so engine
...
- **README** — polished per the zenodotus gate re-run (ideate-core#38, docs only): tightened
...
- **README Quick Start** now documents the two contractual shapes a first
  ...
  the snippet with a prerequisite callout. (ideate-core#30)
