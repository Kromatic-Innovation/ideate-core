 ...
 *                                        reply, or has no client. Makes the swallowed
 *                                        per-agent failure observable; never changes
 *                                        the no-throw robustness contract. Failures are
 *                                        ALSO recorded by default (whether or not this
 *                                        is supplied) in meta.agentErrors, alongside
 *                                        meta.agentsAttempted / agentsFailed -- so total
 *                                        failure is distinguishable from a legitimately
 *                                        empty pool without opting in to this callback.
...
export async function ideateCore(input = {}, deps = {}) {
  ...
  const r1Results = await Promise.all(
    agents.map((agent) => {
      ...
      if (!complete) {
        ...
        if (onAgentError) {
          onAgentError(
            new Error(`no client resolvable for model "${agent.model || "(default)"}"`),
            {
              agentId: agent.id,
              round: 1,
            },
  ...
  const meta = {
    ...
    agentsFailed,
    // The reason behind agentsFailed, recorded by default (ideate-core#154) --
    // one entry per failed agent, always populated regardless of whether the
    // caller supplied deps.onAgentError. agentsFailed === agentsAttempted with
    // candidates.length === 0 is total failure; agentErrors says why.
    agentErrors,
