...
// merged pool then passes an ID-KEYED COLLISION GUARD before the next round /
// selection — not a similarity collapse. Each regenerated idea carries a fresh
// id derived from the original, so it never collapses against the idea it
// revises. Collapsing near-duplicate text, and clustering, belong to the
// optional convergence stage, which runs once after this loop — never inside it.
...
export function normalizeFeedback(raw) {
  ...
  for (const v of rawVerdicts) {
    ...
    const decisionRaw = String(v.decision || v.verdict || "keep")
      .toLowerCase()
      .trim();
 ...
 *   @param {number}   [deps.regenConcurrency]  max concurrent regeneration calls.
 *                                        Default unbounded (mirrors round 1); set a
 *                                        cap if a provider rate-limits.
 *   @param {function} [deps.onEvaluatorError] (err, ctx)=>void. Called when the
 *                                        evaluator throws; the loop still resolves
 *                                        (robustness), and feedback.evaluatorFailures
 *                                        counts the throws so a broken evaluator is
 *                                        observable, not read as a satisfied one.
 * @param {object} [opts]  { context }
 * @returns {Promise<{candidates, feedback:{rounds, history, evaluatorDistinct, evaluatorFailures}}>}
 */
export async function runFeedbackLoop(candidates, deps = {}, opts = {}) {
  ...
  // Optional cap on concurrent regeneration calls (for provider rate limits).
  // Default is unbounded, matching round 1's Promise.all — no unmeasured cap.
  const regenConcurrency =
    Number.isFinite(deps.regenConcurrency) && deps.regenConcurrency > 0
      ? Math.floor(deps.regenConcurrency)
      : Infinity;
  const resolveComplete = makeResolveComplete(deps);
  // Resolve the panel once so regeneration can recover an agent's NUMERIC
  // temperature by original.agentId. A candidate's own `temperature` field holds
  // the persona LABEL (a string), not a number, so it can never be forwarded as
  // a sampling temperature. Keyed on agentId — sound now that ids are unambiguous
  // (ideate-core#87).
  const agentsById = new Map(resolveAgents(deps).map((a) => [a.id, a]));

  let pool = pool0;
  const history = [];
  // A broken evaluator that THROWS is swallowed by safeEvaluate and returns no
  // verdicts — previously indistinguishable from an evaluator that judged the
  // pool and was satisfied. Count the failures so the difference is observable
  // (the ideate-core#90 safeEvaluate gap).
  let evaluatorFailures = 0;
  for (let round = 1; round <= maxRegen; round++) {
    const raw = await safeEvaluate(deps.evaluate, pool, { context, round }, deps.onEvaluatorError);
    if (raw && raw.__evaluatorError) evaluatorFailures++;
    ...
    // Regenerate flagged ideas CONCURRENTLY (mirroring round 1's Promise.all),
    // not one awaited call at a time. Output order is preserved (results are
    // index-aligned to toRevise), and the never-lose-a-flagged-idea semantics
    // hold: a failed regen keeps the original. Concurrency is unbounded by
    // default like round 1; a caller worried about provider rate limits can cap
    // it with deps.regenConcurrency (no default cap is imposed — that would be an
    // unmeasured guess).
    const regenerated = await mapWithConcurrency(
      toRevise,
      regenConcurrency,
      async ({ original, verdict }) => {
  ...
  return {
    candidates: pool,
    feedback: { rounds: history.length, history, evaluatorDistinct, evaluatorFailures },
  };
}
...
async function regenerateOne(original, verdict, poolDirectives, ctx) {
  ...
  // Honor deps.maxTokens and the originating agent's tuned temperature, keeping
  // the previous hardcoded values ONLY as fallbacks. Never forward
  // original.temperature — that is the persona label (a string), not a number.
  const agent = ctx.agentsById ? ctx.agentsById.get(original.agentId) : null;
  const temperature = agent && Number.isFinite(agent.temperature) ? agent.temperature : 0.7;
  const maxTokens = Number.isFinite(ctx.maxTokens) ? ctx.maxTokens : 2048;
  let res;
  try {
    res = await complete({
      prompt,
  ...
  const first = Array.isArray(raw)
    ? raw.find((r) => r && typeof r.text === "string" && r.text.trim())
    : null;
...
async function safeEvaluate(evaluate, pool, ctx, onError) {
  try {
    return await evaluate(pool, ctx);
  } catch (err) {
    // Don't sink the loop on a broken evaluator (robustness), but tag the result
    // so the caller can tell "evaluator threw" apart from "evaluator was
    // satisfied" — the ideate-core#90 safeEvaluate gap. The __evaluatorError marker never
    // reaches a caller: runFeedbackLoop consumes it into feedback.evaluatorFailures.
    if (typeof onError === "function") onError(err, ctx);
    return { verdicts: [], __evaluatorError: true };
  }
}

/**
 * Map `fn` over `items` with at most `limit` in flight, preserving OUTPUT ORDER
 * (results are index-aligned to `items`). With an unbounded limit this is exactly
 * `Promise.all(items.map(fn))`; a finite limit runs a fixed pool of workers.
 */
